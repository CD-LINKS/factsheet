...
if(cfg$r=="JPN"){
  indc_cats <- c("NoPOL","NPi","INDCi","2020_low","2030_high")
} else  {
  indc_cats <- c("NoPOL","NPi","INDCi","2030_low","2030_high","2020_verylow")
}
  nat_cats <- intersect(unique(all[Scope=="national"]$Category),indc_cats)

plot_line(reg=cfg$r,dt=all,vars=c("GDP|MER","Population"),cats=c("INDCi"),title="Socio-Economic Indicators",file_pre="gdp_pop_line")   
...
plot_line(reg=cfg$r,dt=all,vars=vars,cats=cats,title="Policy Costs in INDC2030i_1600 scenarios",file_pre="cons_gdp_loss_line")   
#}
...
cats <- nat_cats #intersect(nat_cats,c("NoPOL","2020_low","2030_low"))
...
```{r, echo=FALSE,results = 'hide',cache=FALSE, message=FALSE, warning=FALSE,fig.width=8, fig.height=10, dpi=200}
if (!identical(character(0),intersect(nat_cats,c("INDCi","2020_low","2030_low")) )){
...
cats <- nat_cats #intersect(nat_cats,c("INDCi","2020_low","2030_low"))
...
```{r, echo=FALSE,results = 'hide',cache=FALSE, message=FALSE, warning=FALSE,fig.width=8, fig.height=10, dpi=200}
if (!identical(character(0),intersect(nat_cats,c("2020_low","2030_low","2030_high")) )){
  # Please specify exactly two variables and assign them to x and y
  vars <- c(x="Price|Carbon",y="relative Abatement|CO2")
  cats <- intersect(nat_cats,c("2020_low","2030_low","2030_high"))
...
```{r, echo=FALSE,results = 'hide',cache=FALSE, message=FALSE, warning=FALSE,fig.width=8, fig.height=10, dpi=200}
if (!identical(character(0),intersect(nat_cats,c("NoPOL","2020_low","2030_low","2030_high","INDCi")) )){
# Please specify exactly two variables and assign them to x and y
vars <- c(x="GDP|MER",y="Final Energy")
cats <- intersect(nat_cats,c("NoPOL","2020_low","2030_low","2030_high","INDCi"))
...
\subsection{Carbon sequestration - Land use}

```{r, echo=FALSE,results = 'hide',cache=FALSE, message=FALSE, warning=FALSE,fig.width=8, fig.height=10, dpi=200}

# For area plots, the first entry in vars is interpreted as the total (line_plot on top)
vars <- c("Carbon Sequestration|Land Use","Carbon Sequestration|Land Use|Afforestation","Carbon Sequestration|Land Use|Biochar","Carbon Sequestration|Land Use|Soil Carbon Management")
cats <- nat_cats
plot_area(reg=cfg$r,dt=all,vars=vars,cats=cats,lab="Carbon sequestration (Mt CO2/yr)",file_pre="cs_landuse_area")   
...
# For area plots, the first entry in vars is interpreted as the total (line_plot on top)
vars <- c("Land Cover","Land Cover|Built-up Area","Land Cover|Cropland", "Land Cover|Forest", "Land Cover|Pasture", "Land Cover|Other Arable Land", "Land Cover|Other Land")
cats <- nat_cats
plot_area(reg=cfg$r,dt=all,vars=vars,cats=cats,lab="Land cover (million ha)",file_pre="Land_cover_area")   

invisible(dev.off())
```

\subsection{Land use - Cropland}

```{r, echo=FALSE,results = 'hide',cache=FALSE, message=FALSE, warning=FALSE,fig.width=8, fig.height=10, dpi=200}

# For area plots, the first entry in vars is interpreted as the total (line_plot on top)
vars <- c("Land Cover|Cropland","Land Cover|Cropland|Cereals","Land Cover|Cropland|Irrigated", "Land Cover|Cropland|Double-cropped", "Land Cover|Cropland|Rainfed", "Land Cover|Cropland|Energy Crops")
cats <- nat_cats
plot_area(reg=cfg$r,dt=all,vars=vars,cats=cats,lab="Land cover cropland (million ha)",file_pre="Cropland_area")   

invisible(dev.off())
```

\subsection{Land use - Agricultural production}

```{r, echo=FALSE,results = 'hide',cache=FALSE, message=FALSE, warning=FALSE,fig.width=8, fig.height=10, dpi=200}

# For area plots, the first entry in vars is interpreted as the total (line_plot on top)
vars <- c("Agricultural Production","Agricultural Production|Energy","Agricultural Production|Non-Energy")
cats <- nat_cats
plot_area(reg=cfg$r,dt=all,vars=vars,cats=cats,lab="Agricultural Production (million t DM/yr)",file_pre="agri_prod_area")   

invisible(dev.off())
```

\subsection{Land use - Yields}

```{r, echo=FALSE,results = 'hide',cache=FALSE, message=FALSE, warning=FALSE,fig.width=8, fig.height=10, dpi=200}

vars <- c("Yield|Cereal","Yield|Oilcrops","Yield|Sugarcrops")
cats <- nat_cats
plot_funnel(reg=cfg$r,dt=all,vars=vars,cats=cats,title="Yield (t DM/ha/yr)",file_pre="yield_funnel",glob_lines=T,xlim=c(2000,2050))   

invisible(dev.off())
```

\subsection{Land use - Kyoto gas emissions}

```{r, echo=FALSE,results = 'hide',cache=FALSE, message=FALSE, warning=FALSE,fig.width=8, fig.height=10, dpi=200}

plot_funnel(reg=cfg$r,dt=all,vars=c("Emissions|CO2|AFOLU","Emissions|CH4|AFOLU","Emissions|N2O|AFOLU"),cats=nat_cats,title="Land use Kyoto gas emissions",file_pre="LU_emis_funnel",glob_lines=T,xlim=c(2000,2050))   
