...
cats_stack_fig1 <- c("2010", "2015", "No new policies","National policies","NDC", "Carbon budget 1000", "Carbon budget 400")
catsnat_fig1 <- c("2010", "2015", "No new policies","National policies","NDC")
...
# use different scenario names for 2C and 1.5C scenario
all_fig1a$Category=str_replace_all(all_fig1a$Category,"Carbon budget 1000","2° C")
all_fig1a$Category=str_replace_all(all_fig1a$Category,"Carbon budget 400", "1.5° C")
...
# use different scenario names for 2C and 1.5C scenario
all_fig1b_h$Category=str_replace_all(all_fig1b_h$Category,"Carbon budget 1000","2° C")
all_fig1b_h$Category=str_replace_all(all_fig1b_h$Category,"Carbon budget 400", "1.5° C")

# 2C and 1.5C scenario names are different in graph
cats_stack_fig1_used <- c("2010", "2015", "No new policies","National policies","NDC")
catsnat_fig1_used <- c("2010", "2015", "No new policies","National policies","NDC")
cats_a1_used <- c("No new policies","National policies", "NDC", "2° C", "1.5° C")
all_fig1a$Category <- factor(all_fig1a$Category, levels=cats_a1_used)
all_fig1b_h$Category <- factor(all_fig1b_h$Category, levels=cats_stack_fig1_used)
...
a1<-plot_funnel2(reg="World",dt=all_fig1a,vars=c("Emissions|Kyoto Gases"),cats=cats_a1_used,start_scen=2010, title="Global greenhouse gas emissions",
...
regs_fig1 <- c("BRA")
b1<-plot_stackbar_ghg(regs=regs_fig1, dt=all_fig1b_h, vars_stack=vars_fig1b_h, var_line="", cats=cats_stack_fig1_used, catsnat=catsnat_fig1_used, 
...
regs_fig1 <- c("CHN")
c1<-plot_stackbar_ghg(regs=regs_fig1, dt=all_fig1b_h, vars_stack=vars_fig1b_h,var_line="", cats = cats_stack_fig1_used, catsnat=catsnat_fig1_used, 
...
regs_fig1 <- c("IND")
d1<-plot_stackbar_ghg(regs=regs_fig1, dt=all_fig1b_h, vars_stack=vars_fig1b_h,var_line="", cats = cats_stack_fig1_used, catsnat=catsnat_fig1_used, 
                      per=c(2030), file_pre="1b_IND_2030", lab = "", title=T, Title="India", 
                      linegraph=F, hist=T,labels=T,x_labels=T,var.labels=var.labels_fig1b_h, legend_name=legend_name_fig1, TotalEmis_var=TotalEmis_var_fig1,
                      natpoints=T, error_bar=T,quantiles=T)

regs_fig1 <- c("JPN")
e1<-plot_stackbar_ghg(regs=regs_fig1, dt=all_fig1b_h, vars_stack=vars_fig1b_h,var_line="", cats = cats_stack_fig1_used, catsnat=catsnat_fig1_used, 
                      per=c(2030), file_pre="1b_JPN_2030", lab = "", title=T, Title="Japan", 
                      linegraph=F, hist=T,labels=T,x_labels=T,var.labels=var.labels_fig1b_h, legend_name=legend_name_fig1, TotalEmis_var=TotalEmis_var_fig1,
                      natpoints=T, error_bar=T,quantiles=T)

regs_fig1 <- c("USA")
f1<-plot_stackbar_ghg(regs=regs_fig1, dt=all_fig1b_h, vars_stack=vars_fig1b_h,var_line="", cats = cats_stack_fig1_used, catsnat=catsnat_fig1_used, 
                      per=c(2030), file_pre="1b_USA_2030", lab = "", title=T, Title="United States", 
                      linegraph=F, hist=T,labels=T,x_labels=T,var.labels=var.labels_fig1b_h, legend_name=legend_name_fig1, TotalEmis_var=TotalEmis_var_fig1,
                      natpoints=T, error_bar=T,quantiles=T)

regs_fig1 <- c("EU")
g1<-plot_stackbar_ghg(regs=regs_fig1, dt=all_fig1b_h, vars_stack=vars_fig1b_h,var_line="", cats = cats_stack_fig1_used, catsnat=catsnat_fig1_used, 
                      per=c(2030), file_pre="1b_EUE_2030", lab = "", title=T, Title="European Union", 
                      linegraph=F, hist=T,labels=T,x_labels=T,var.labels=var.labels_fig1b_h, legend_name=legend_name_fig1, TotalEmis_var=TotalEmis_var_fig1,
                      natpoints=T, error_bar=T,quantiles=T)

regs_fig1 <- c("RUS")
h1<-plot_stackbar_ghg(regs=regs_fig1, dt=all_fig1b_h, vars_stack=vars_fig1b_h,var_line="", cats = cats_stack_fig1_used, catsnat=catsnat_fig1_used, 
...
plot(h_fig1)
ggsave(file=paste("NatComPaper/graphs","/Figure1_NatCom.jpg",sep=""),h_fig1,width=20,height=12,dpi=300)

# FIGURE 2
regs_paper<- c("World", "BRA",  "CHN", "EU",  "IND", "JPN", "RUS", "USA", "World")
# define variables and scenarios to show in graph
vars_stack_fig2 <- c("Final Energy|Transportation", "Final Energy|Residential and Commercial","Final Energy|Industry", "Final Energy|Other Sector")
vars_line_fig2 <- c("Final Energy|Non-fossil share")
cats_stack_fig2 <- c("2010", "2015", "No new policies","National policies","NDC")
catsnat_fig2 <- c("2010", "2015", "No new policies","National policies","NDC")
var.labels_fig2 <- c("Transport", "Buildings", "Industry", "Other")
...
h_fig2=grid.arrange(a2,c2,f2,g2,legend2_1,d2,b2,e2,h2, layout_matrix=lay_fig2) #ncol=3
ggsave(file=paste("NatComPaper/graphs","/Figure2_NatCom.jpg",sep=""),h_fig2,width=20,height=12,dpi=300)
...
ggsave(file=paste("NatComPaper/graphs","/Figure1_alt_GHG_FE_NatCom.jpg",sep=""),h_fig12,width=20,height=12,dpi=300)
...
Gap_stat <- filter(data_fig3_stat, period==2030) %>% 
            select(-perc_10, -perc_90) %>%
            spread(key=Category, value=median) %>%
            mutate(start_median = `National policies`, 
                   gap_2C_median=`National policies`-`Carbon budget 1000`,
...
regs_fig3_labels <- c("BRA"="Brazil","CHN"="China","IND"="India","EU"="EU",
                         "JPN"="Japan","USA"="USA","RUS"="Russia","RoW"="Rest of World","World"="World")
mods_fig3 <- unique(all_paper$model)
vars_fig3 <- "Emissions|CO2"
cats_fig3 <- c("No new policies","National policies","NDC","Carbon budget 1000","Carbon budget 400")
cats_fig3_nat <- c("No new policies", "National policies", "NDC","Carbon budget 1000","Carbon budget 400")
...
write.table(v_emi_cumrel, file = "NatComPaper/data/EmissionBudgets.csv", sep=";", row.names = F)

v_emi_cumrel_stat <- group_by(v_emi_cumrel, scenario, Category, Scope, region, variable) %>%
                     summarise(median=median(value), min=min(value), max=max(value))
write.table(v_emi_cumrel_stat, file = "NatComPaper/data/EmissionBudgets_stat.csv", sep=";", row.names = F)
...
b=ggplot() +
  ...
  # PRIMES INDC model only contains GHG emisisons until 2030, so left out
  geom_point(data=v_plot[variable=="CO2rel2010" & model!="*PRIMES_V1"],aes(x=Category,y=value,shape=model,color=model,size=model)) + # coord_cartesian(ylim = c(0, 100)) + & (Scope=="global" | Scope=="national" & Category%in%cats_fig3_nat
  facet_wrap(~region, scales = "free_y", labeller=labeller(region=regs_fig3_labels)) +
  ...
  xlab("") + ylab("")  +
  scale_color_manual(values = c(rep("orangered3",10),rep("black",9)),name="Model")+
  scale_shape_manual(values = rep(seq(1,10),2),name="Model") +
  scale_size_manual(values = c(rep(2,9), rep(3,10)),name="Model") +
  scale_fill_manual(values=plotstyle(cats_fig3), name="Scenario", labels=c("No new policies"="No new policies", 
...
# FIGURE 3a and 3b (ALTERNATIVE) --> used in Supplementary Informatino of NatCom paper

# ALTERNATIVE Figure 3a (Figure 1 SI)
cats_fig3 <- c('National policies', 'Carbon budget 1000', 'Carbon budget 400')
...
data_fig3a$value <- data_fig3a$value/1000
...
data_fig3a_hist$value <- data_fig3a_hist$value/1000
...
# instead of median of gaps, take gap of median scenario values (corresponds with arrows in graph)
Gap3_1_stat <- filter(data_fig3a_stat, period==2030) %>% 
  select(-perc_10, -perc_90) %>%
  spread(key=Category, value=median) %>%
  mutate(start_median = `National policies`, 
         gap_2C_median=`National policies`-`Carbon budget 1000`,
...
var_labels_a <- c("Emissions|Kyoto Gases"="Total GHG", 
                  ...
                  "Emissions|CO2|Industrial Processes"="CO2 ind. processes", 
...
fig3a <- ggplot(data=data_fig3a_stat) + geom_line(aes(period, median, colour=Category), size=2) + 
  ...
  geom_segment(data=Gap3_1_stat, mapping=aes(x=2031, y=start_median, xend=2031, yend=start_median-gap_2C_median), 
               arrow=arrow(length = unit(0.25, "cm")), size=1)+#, color="dark blue") +
  geom_segment(data=Gap3_1_stat, mapping=aes(x=2035, y=start_median, xend=2035, yend=start_median-gap_1.5C_median), 
               arrow=arrow(length = unit(0.25, "cm")), size=1)+#, color="dark blue") +
...
ggsave(file=paste("NatComPaper/graphs","/Figure1_SI_NatCom.jpg",sep=""),fig3a,width=20,height=12,dpi=300)

# ALTERNATIVE FIGURE 3B (Suppplementary Figure 2)
...
var_labels_b <- c("Energy intensity of GDP"="Energy intensity (TJ/$US2010)", 
                  "Final Energy|Non-fossil share"="Low carbon share (%)", 
                  #"Fossil fuel utilisation rate"="TPES_ff/FF_ff",
                 "Carbon intensity of fossil-fuel use"="Carbon intensity (MtCO2/EJ)")
...
# instead of median of gaps, take gap of median scenario values (corresponds with arrows in graph)
Gap3_2_stat <- filter(data_fig3b_stat, period==2030) %>% 
  select(-perc_10, -perc_90) %>%
  spread(key=Category, value=median) %>%
  mutate(start_median = `National policies`, 
         gap_2C_median=`National policies`-`Carbon budget 1000`,
...
fig3b <- ggplot(data=data_fig3b_stat) + geom_line(aes(period, median, colour=Category), size=2) + 
  ...
  geom_segment(data=Gap3_2_stat, mapping=aes(x=2032, y=start_median, xend=2032, yend=start_median-gap_2C_median), 
               arrow=arrow(length = unit(0.25, "cm")), size=1)+#, color="dark blue") +
  geom_segment(data=Gap3_2_stat, mapping=aes(x=2031, y=start_median, xend=2031, yend=start_median-gap_1.5C_median), 
               arrow=arrow(length = unit(0.25, "cm")), size=1)+#, color="dark blue") +
  ...
  facet_wrap(variable~region, scales = "free_y", nrow=3, labeller=labeller(variable = var_labels_b, region=reg_labels)) +
  #https://stackoverflow.com/questions/42588238/setting-individual-y-axis-limits-with-facet-wrap-not-with-scales-free-y/42590452
...
ggsave(file=paste("NatComPaper/graphs","/Figure2_SI_NatCom.jpg",sep=""),fig3b,width=20,height=12,dpi=300)
...
scens_decomp <- c('No new policies', 'National policies')
...
decomposition <- transmute(decomposition, model=model, region=region,
                           ...
                           EM_growth_NoPolicy=`No new policies_Emissions|Kyoto Gases_2030_value`-`No new policies_Emissions|Kyoto Gases_2015_value`,
                           Policy_implementation=`No new policies_Emissions|Kyoto Gases_2030_value`-`National policies_Emissions|Kyoto Gases_2030_value`)
...
decomposition_median <- group_by(decomposition, driver, region) %>%
  summarize(median=median(value,na.rm=T),
            min=quantile(value,prob=0.1,na.rm=T),max=quantile(value,prob=0.9,na.rm=T)) %>%
  as.data.frame()
...
p_drivers_world_median <- ggplot(data=decomposition_world_median) + 
  ...
  scale_fill_manual(name = "Driver",
                    ...
                    breaks=c("EM_growth_NPi.model", 
                             ...
                             "Error_term.model"),
                    labels=c("EM_growth_NPi.model"="Emission growth in National policies scenario (2015-2030)", 
                             "EM_history.model"="Differences in historical (2015) emissions", 
                             "EM_growth_NoPolicy.model"="Different assumptions on growth of socio-economic factors",
                             "Policy_implementation.outside"="Policy impact not covered by model", 
                             "Policy_implementation.model"="Different impact of policy implementation ",
                             "Error_term.model"="Uncertainty as a result of model form")
  ) +
