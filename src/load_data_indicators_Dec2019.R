...
source("functions/data_processing_indicators.R")
...
if (file.exists(paste0("data/",cfg$infile,reg,"_proc.Rdata")) & !b.procdata) {
  ...
  all[MODEL %in% c("IMAGE 3.0") & (SCENARIO!="NPip_V4")]$SCENARIO <- str_replace_all(all[MODEL %in% c("IMAGE 3.0") & (SCENARIO!="NPip_V4")]$SCENARIO,"_V4","_V4real")
  ...
  all[MODEL %in% c("AIM/Enduse 3.0","AIM/Enduse[Japan]","China TIMES","COPPE-COFFEE 1.0","COPPE-MSB_v2.0","DNE21+ V.14 (national)","GCAM-USA_CDLINKS","IPAC-AIM/technology V1.0","PRIMES_V1","REMIND-MAgPIE 1.7-3.0")]$SCENARIO <- str_replace_all(
  all[MODEL %in% c("AIM/Enduse 3.0","AIM/Enduse[Japan]","China TIMES","COPPE-COFFEE 1.0","COPPE-MSB_v2.0","DNE21+ V.14 (national)","GCAM-USA_CDLINKS","IPAC-AIM/technology V1.0","PRIMES_V1","REMIND-MAgPIE 1.7-3.0")]$SCENARIO,"_V3","_V4")
...
all_tmp <- all 
  ...
  #### add variables
  all_import_before_add_variables <- all
  if(addvars){
