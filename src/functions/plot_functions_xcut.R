...
plot_boxplot <- function(regs, dt, vars, cats, year = 2050, out=cfg$outdir, title="Title", file_pre="boxplot",connect=T,
  ...
  dt <- dt[ Variable %in% vars & Year == year & Category %in% cats & Region %in% regs & !is.na(value)] %>% factor.data.frame()
  ...
  dtg <- dt[Scope=="global" & Variable %in% vars & Year == year & Category %in% cats & Region %in% regs]  %>%
    rename(Global = Model )
  dtn <- dt[Scope=="national" & Variable %in% vars & Year == year & Category %in% cats & Region %in% regs] %>%
