...
plot_funnel2 <- function(reg, dt, vars, cats, start_scen, out=cfg$outdir, title="Title", file_pre="def",ylim=NA,xlim=NA,glob_lines=F,range=F,
  ...
  #}
  #select data
  dt <- dt[region==reg & Category%in% cats & variable%in% vars & period>=start_scen & !is.na(value)]
           #create string with y-axis units for axis label
  unitsy <- paste0("(",unique(dt[variable%in%vars]$unit),")    ")
  unitsy <- paste(rev(unitsy),sep='',collapse='')
  # For each variable count models and add to data and variable name
  ...
  # historical data
  if (hist){
    p = p + geom_point(data=filter(dt_hist, region==reg),aes(x=period,y=value))
  }
  # scenarios
  if (glob_lines){
     p = p + geom_path(data=dt[region==reg & Scope=="global" & period>=2010],aes(x=period,y=value,group = interaction(scenario,model),
