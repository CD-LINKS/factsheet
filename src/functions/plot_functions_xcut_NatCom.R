...
plot_stackbar_ghg <- function(regs, dt, vars_stack, var_line="", cats, catsnat, 
                              per, out=cfg$outdir, lab="",lab_line="", title=F,Title="Title",file_pre="stackbar",
                              ylim=NA,ybreaks=NA,
                              linegraph=F, hist=F,labels=F, x_labels=F, var.labels=NA, legend_name=NA, TotalEmis_var=NA, 
                              natpoints, error_bar=F,quantiles=T, minprob=0.1,maxprob=0.9,
                              color_brewer_palette="Spectral", add_legend=""){

  # change unit of emissions to GtCO2eq
  ...
  dta$variable <- factor(dta$variable, levels = vars_stack, ordered = T)
  ...
  if (linegraph==TRUE) {
    dtline <- filter(dtline, region %in% regs, Category%in% cats, variable%in% var_line, period %in% per,Scope=="global")
  ...
  else {  p = p + scale_y_continuous(name=lab)}
  
  # add additional legend
  p = p + geom_line(aes(x=c(0,0,0,0,0),y=c(0,0,0,0,0),linetype="blank"))
  p = p + scale_linetype_discrete(name="", labels=c(add_legend))
  ...
  if(title){p = p + ggtitle(Title)}
  if(!x_labels){p = p + scale_x_discrete(labels=NULL)}
  if (!all(is.na(ylim))){p = p + scale_y_continuous(limits=ylim,breaks=ybreaks)} #manual y-axis limits
  if(labels){p = p + scale_fill_brewer(palette=color_brewer_palette,labels=var.labels,name=legend_name)}else{p = p + scale_fill_brewer(palette=color_brewer_palette,name=legend_name)}
  ggsave(file=paste0(out,"/",file_pre,"_",per,cfg$format),p, width=7, height=8, dpi=120)
  return(p)
}
