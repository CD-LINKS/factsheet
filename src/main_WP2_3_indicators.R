...
# STRUCTURE
# source("settings/config_xCut.R") - defines regions and national models
# source functions process_data() and add_variables() - source("functions/data_processing.R")
# source file with plot functions - source("functions/plot_functions.R")

# - infile cdlinks_compare_20171028-093027
# - variable selection: variables_xCut.csv --> results is 'all'
# - distinction made between scenario and Category, for one Category, different scenario versions (V1, V2, V3, V4) exist
# - process data / factor.data.frame - changes sturcture of dataset 'all'
# - adjust reporting - add mising data, correct data (adjust_reporting_indc_Mark.R)
# - add variables (from data_processing.R) - add additional variables with calcVariable, calcRel2Base, calcBudget, calcPeak, calcRate, 
...
if (file.exists(paste0("data/",cfg$infile,"_proc.Rdata")) & !b.procdata) {
  ...
  all_import_adjusted <- all
  
  #### from raw wide format to long format with additional columns
  cat("- process data into correct format\n")
  ...
  cat("- adjust data for missing variables\n")
  #all_duplicated <- all[duplicated(all),]
  #if (dim(all_duplicated)[1]>0) {cat("Warning: duplicated rows, check all_duplicated\n")}
...
all$Category=str_replace_all(all$Category,"2020_high","Carbon budget 1600")
