...
regions_incl_bunkers <- c("BRA", "CHN", "EU", "IND", "JPN", "RUS", "USA", "World", "ROW", "Bunkers")
...
write.table(GHG_intensity_hist, paste0("Indicators/data/GHG_intensity_", GDP_MER_PPP, "_hist.csv"), sep=";", row.names=F)
...
write.table(d_selec_Kyoto_stat, file="Indicators/data/figure1.csv", sep=";", row.names = FALSE)
...
d_cd_links_GHG_countries_stat$region <- factor(d_cd_links_GHG_countries_stat$region, levels=regions_incl_bunkers)
# determine RoW category
...
d_cd_links_GHG_countries_stat_fig2 <- mutate(d_cd_links_GHG_countries_stat, source="CD-LINKS")
...
Countries_Gases_Kyoto <- gather(d_selec_Kyoto_stat, 7:ncol(d_selec_Kyoto_stat), key="year", value=value) %>% as.data.frame()
...
d_cd_links_GHG_countries_stat$region <- factor(d_cd_links_GHG_countries_stat$region, level=regions_incl_bunkers)
...
if (GDP_MER_PPP == "PPP") { intens=filter(all_cd_links, variable%in%c("Carbon Intensity of GDP|PPP (excl AFOLU)","GHG Intensity of GDP|PPP"),
                            scenario %in% scens, region %in% regions, region!="Bunkers", year<=2050)
} else {intens=filter(all_cd_links, variable%in%c("Carbon Intensity of GDP|MER (excl AFOLU)","GHG Intensity of GDP|MER"),
                      scenario %in% scens, region %in% regions, region!="Bunkers", year<=2050)
}
...
intens_stat <- group_by(intens, scenario, region, year, variable, unit) %>% summarise(mean=mean(value,na.rm=TRUE),
                                                                                       ...
                                                                                       tenp=quantile(value, .10, na.rm=TRUE),
                                                                                       ninetyp=quantile(value, .90, na.rm=TRUE))
write.table(intens_stat, paste0("Indicators/data/GHG_intensity_", GDP_MER_PPP, "_stat.csv"), sep=";", row.names=F)
...
write.table(intensrate_stat,"Indicators/data/figure4.csv", sep=";", row.names = FALSE)
...
data_figure5_CO2budget <- filter(all_cd_links, scenario %in% scens, region %in% regions, year==2100, variable=="Carbon budget")
...
INDC2030i_1000_fig6_elec <- filter(INDC2030i_1000_indicators$NonFossilElecShare, year>=2010, year<=2050) %>% mutate(scenario="2C_66 (2030)") %>% mutate(sector="Electricity")
NPi2020_1600_fig6_elec <- filter(NPi2020_1600_indicators$NonFossilElecShare, year>=2010, year<=2050) %>% mutate(scenario="2C_50") %>% mutate(sector="Electricity")
...
INDC2030i_1000_fig6_res <- filter(INDC2030i_1000_indicators$NonFossilResBuildingsShare, year>=2010, year<=2050, population_group=="Total") %>% mutate(scenario="2C_66 (2030)", sector="Residential buildings")
...
INDC2030i_1000_fig6_transport <- filter(INDC2030i_1000_indicators$NonFossilTransportShare, year>=2010, year<=2050, travel_mode=="Total", type=="Total") %>% mutate(scenario="2C_66 (2030)") %>% mutate(sector="Transport")
...
# FIGURE 9
data_figure9 <- filter(all_cd_links, value>0, scenario %in% scens, region %in% regions, variable%in%c("Land Cover|Forest"))
...
d7 <- read.table("Indicators/data/data_figure7.csv", header=TRUE, sep=";")
...
write.table(gaps_stat, "Indicators/data/gaps_stat.csv", sep=";", row.names=F)
