...
tmp1 <- v_emireg %>% filter(variable %in% c("Carbon budget", "Carbon budget|Energy and Industry", "Carbon budget|Energy"), period == 2030)
...
write.csv(v_emi_cumrel, file = "EmissionBudgets.csv", row.names = F,
          col.names = c("MODEL", "SCENARIO", "REGION", "CO2 Energy&Ind 2010",  "CO2 E&I 2010-2030", "Emission Years E&I",
                        "CO2 total 2010",  "CO2 total 2010-2030", "Emission Years CO2 total"))
...
ggplot() +
    ...
    ggtitle(paste0(" Cumulative CO2 (incl. AFOLU) 2011-2030")) + ylab("Gt CO2") +
...
ggplot() +
  ...
  geom_hline(data=ref_budgets,aes(yintercept=value),colour=rep(c("#aa0000","#0000aa"),9))+  #
    facet_wrap(~region, scales = "free_y") +
    ggtitle(paste0(" Cumulative CO2 Energy and Industry 2011-2030")) + ylab("Gt CO2") +
  scale_color_manual(values = c(rep("black",10),rep("red",10)))+
...
ggplot() +
  ...
  geom_hline(data=ref_budgets,aes(yintercept=value),colour=rep(c("#aa0000","#0000aa"),9))+  #
  facet_wrap(~region, scales = "free_y") +
  ggtitle(paste0(" Cumulative CO2 Energy 2011-2030")) + ylab("Gt CO2") +
  scale_color_manual(values = c(rep("black",10),rep("red",10)))+
  scale_shape_manual(values = c(seq(1,10),seq(1,10))) +
  scale_size_manual(values = c(rep(1,10),rep(3,10))) +
  theme(axis.text.x  = element_blank() )
...
ggplot() +
  ...
  geom_point(data=v_plot,aes(x=scenario,y=`FFIrel2010`,shape=model,color=model,size=model)) +
    facet_wrap(~region, scales = "free_y") +
    ggtitle(paste0("CO2 Energy and Industry (2011-2030 rel. to 2010)")) + ylab("Emission Years") +
  scale_color_manual(values = c(rep("black",10),rep("red",10)))+
  scale_shape_manual(values = rep(seq(1,10),2)) +
  scale_size_manual(values = c(rep(1,10),rep(3,10))) +
    theme(axis.text.x  = element_blank() )
...
ggplot() +
  ...
  geom_point(data=v_plot,aes(x=scenario,y=`CO2rel2010`,shape=model,color=model,size=model)) +
    facet_wrap(~region, scales = "free_y") +
    ggtitle(paste0("CO2 total (2011-2030 rel. to 2010)")) + ylab("Emission Years") +
  scale_color_manual(values = c(rep("black",10),rep("red",10)))+
  scale_shape_manual(values = rep(seq(1,10),2)) +
  scale_size_manual(values = c(rep(1,10),rep(3,10))) +
    theme(axis.text.x  = element_blank() )
